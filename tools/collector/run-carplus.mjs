// run-carplus.mjs — CLI da recolha batch do carplus.pt.
//
// Uso:
//   node run-carplus.mjs --max-pages 3                    # amostra (listagem geral)
//   node run-carplus.mjs --brand audi --max-pages 2       # só uma marca (path /carros-usados/{slug}/)
//   node run-carplus.mjs --full --max-pages 120           # cobertura fatiada por marca
...
import { HttpClient } from './carplus/http.mjs';
import { crawl } from './carplus/crawl.mjs';
...
async function main() {
  ...
  console.log(`=== carplus.pt${slice ? ` | ${slice}` : ''}`
  ...
  writeFileSync(join(outDir, 'carplus-summary.json'), JSON.stringify(summary, null, 2));
  ...
  console.log(`top marcas: ${top(stats.byBrand)}`);
  console.log(`top stands: ${top(stats.bySource)}`);
  console.log(`\nNDJSON → ${ndjsonPath}`);
  console.log(`resumo → ${join(outDir, 'carplus-summary.json')}`);
}
