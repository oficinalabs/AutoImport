// autocasion/parse.ts — extração dos dados de uma página de listagem do autocasion.com.
//
...
import { normalizeProduct, type RawProduct, type CardExtras, type AutocasionRecord } from './schema.ts';

// Resultado do parse de uma página de listagem.
export interface ParsedPage {
  listings: AutocasionRecord[];
  total: number | null;
}
...
export function extractProducts(html: string): RawProduct[] {
...
function parseCardUl(chunk: string) {
  let lis: string[] | null = null;
...
function parseCardDealer(chunk: string) {
...
export function extractCardExtras(html: string): Map<number, CardExtras> {
  const map = new Map<number, CardExtras>();
...
export function recordId(rec: AutocasionRecord): string | null {
