// run-autocasion.mjs — CLI da recolha batch do autocasion.com.
//
// Uso:
//   node run-autocasion.mjs --max-pages 3                 # amostra (listagem geral)
//   node run-autocasion.mjs --brand audi --max-pages 2    # só uma marca (via SEO /…-ocasion)
...
import { HttpClient } from './autocasion/http.mjs';
import { crawl } from './autocasion/crawl.mjs';
...
async function main() {
  ...
  console.log(`=== autocasion.com${args.brand ? ` | marca ${args.brand}` : ''}`
  ...
  writeFileSync(join(outDir, 'autocasion-summary.json'), JSON.stringify(summary, null, 2));
  ...
  console.log(`\nNDJSON → ${ndjsonPath}`);
  console.log(`resumo → ${join(outDir, 'autocasion-summary.json')}`);
}
