// autohero/http.ts — cliente HTTP do autohero.com (retalhista de usados do grupo AUTO1).
//
...
import type { SearchVariables, AdsResponse } from './parse.ts';
...
export class HttpClient extends BaseClient {
  ...
  // `variables` é o objeto `{ search: {...} }` (ver parse.ts). Devolve o JSON.data.searchAdV9AdsV2
  // (o objeto `{ total, data[] }`) ou null se esgotar as tentativas / a resposta for inútil.
  async postGraphql(variables: SearchVariables): Promise<AdsResponse | null> {
    ...
    for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
      ...
      try {
        ...
        if (!res.ok) { lastErr = `HTTP ${res.status}`; }
          ...
          const json = await res.json().catch(() => null) as { data?: { searchAdV9AdsV2?: AdsResponse }; errors?: { message?: string }[] } | null;
