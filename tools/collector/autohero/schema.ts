// autohero/schema.ts — schema-alvo comum + mapeamento de um anúncio da API do autohero.com.
//
...
// Registo do autohero = os campos-base comuns + extras ricos que a API oferece de graça.
export interface AutoheroRecord extends CollectorRecord {
  source_site: string;
  id: string | null;
  stock_number: string | null;
  retail_ad_id: string | null;
  ...
  retail_ad_state: string | null;
}

// Valor monetário da API: `{ amountMinorUnits, conversionMajor, currency }` (cêntimos).
interface Money { amountMinorUnits?: number; conversionMajor?: number; currency?: unknown }

// Forma mínima de um anúncio de `searchAdV9AdsV2.data[]` que consumimos. Payload externo
// dinâmico → só tipamos o que lemos, tudo opcional, e narrowing no acesso.
interface RawAd {
  id?: string;
  manufacturer?: unknown;
  model?: unknown;
  subType?: unknown;
  subTypeExtra?: unknown;
  firstRegistrationYear?: number;
  registration?: unknown;
  builtYear?: number;
  mileage?: { distance?: unknown };
  fuelType?: number;
  gearType?: number;
  ccm?: number;
  kw?: number;
  ...
  ahMainImageUrl?: unknown;
  stockNumber?: unknown;
  retailAdId?: unknown;
  driveTrain?: unknown;
  co2Value?: number;
  fuelConsumption?: { combined?: number };
  numberOfAccidents?: number;
  numberOfDamages?: number;
  carPreownerCount?: number;
  hasFilledServiceBook?: unknown;
  emissionSticker?: unknown;
  monthlyPayment?: Money;
  previousPrice?: Money;
  firstPrice?: Money;
  esBranch?: { city?: unknown; zipcode?: unknown; name?: unknown };
  firstPublishedAt?: unknown;
  publishedAt?: unknown;
  isComingSoon?: unknown;
  retailAdState?: unknown;
}
...
const GEAR_MAP: Record<string, string> = { 1138: 'Manuell', 1139: 'Automatik', 1140: 'Halbautomatik', 1141: 'Doppelkupplung' };

// Preço em unidades maiores (EUR) a partir de `{ amountMinorUnits, conversionMajor }` (cêntimos).
function toMajor(money: Money | null | undefined): number | null {
...
function imageUrl(a: RawAd): string | null {
...
export function normalizeAd(a: RawAd, { collectedAt = null }: { collectedAt?: string | null } = {}): AutoheroRecord {
  ...
  return {
    ...
    fuel: FUEL_MAP[String(a.fuelType ?? '')] || (a.fuelType != null ? String(a.fuelType) : null),
    gearbox: GEAR_MAP[String(a.gearType ?? '')] || (a.gearType != null ? String(a.gearType) : null),
