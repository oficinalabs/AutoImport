// watch.mjs — recolha CONTÍNUA (polling) da página de anúncios mais recentes.
//
...
// o histórico de dedupe/preços). O envio para a DB fica isolado em sink.mjs.
...
export async function watch(config) {
  ...
  const state = new Map(existsSync(statePath)
