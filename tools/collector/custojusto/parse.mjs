// custojusto/parse.mjs — extrai os dados de uma página de listagem do CustoJusto.pt.
//
...
// Extraímos a marca casando o título contra a taxonomia `carBrands` (75 marcas) que vem no próprio
// SSR — robusto para marcas multi-palavra (Mercedes-Benz, Alfa Romeo, Land Rover). Ver schema.mjs.

import { normalizeListing, buildBrandMatcher } from './schema.mjs';
...
// Taxonomia de marcas do SSR (`initialState.search.options.carBrands` → [{name, shortName}]).
// Serve para (a) casar a marca no título e (b) semear o modo --full.
export function extractCarBrands(html) {
  const nd = extractNextData(html);
  const arr = nd?.props?.pageProps?.initialState?.search?.options?.carBrands;
  if (!Array.isArray(arr)) return [];
  return arr.map((b) => b?.brand).filter((b) => b?.shortName)
...
export function extractDistricts(html) {
  const nd = extractNextData(html);
...
export function parseListingPage(html, { collectedAt = null, brandHint = null } = {}) {
  const nd = extractNextData(html);
