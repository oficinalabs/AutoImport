// autoboerse/watch.ts — recolha CONTÍNUA (polling) do autoboerse.de. Mesma lógica do
...
type WatchRow = AutoboerseRecord & { first_seen: string; last_seen: string };
...
export async function watch(config: WatchConfig) {
  ...
  const statePath = join(outDir, 'autoboerse-state.json');
  const sink = new Sink(outDir, 'autoboerse');
  ...
  console.log(`▶ watch autoboerse.de | ${pages} pág recentes | intervalo ${intervalMs / 1000}s`
