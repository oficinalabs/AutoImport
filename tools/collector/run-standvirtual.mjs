// run-standvirtual.mjs — CLI da recolha batch do standvirtual.com.
//
// Uso:
//   node run-standvirtual.mjs --max-pages 3                     # amostra (listagem geral)
//   node run-standvirtual.mjs --brand bmw --max-pages 5         # só uma marca (slug do path)
//   node run-standvirtual.mjs --full                            # cobertura completa (~1324 pág)
...
import { crawl } from './standvirtual/crawl.mjs';
...
async function main() {
  ...
  const http = new HttpClient({ minDelayMs: Number(args.rate) || 2500 });

  console.log(`=== standvirtual.com${args.brand ? ` | marca ${args.brand}` : ''}`
  ...
  writeFileSync(join(outDir, 'standvirtual-summary.json'), JSON.stringify(summary, null, 2));
  ...
  console.log(`stand vs particular: ${top(stats.bySellerType)}`);
  console.log(`top regiões: ${top(stats.byRegion)}`);
  console.log(`top stands: ${top(stats.bySource)}`);
  console.log(`\nNDJSON → ${ndjsonPath}`);
  console.log(`resumo → ${join(outDir, 'standvirtual-summary.json')}`);
}
