// olxpt/parse.ts — extrai os anúncios de uma página de listagem SSR do olx.pt e constrói as URLs.
//
...
import { normalizeAd, type OlxptRecord } from './schema.ts';
import { BASE, CAT_PATH } from './http.ts';
...
export function extractState(html: string): unknown {
...
export function parseListingPage(html: string, { collectedAt = null, forcedMake = null }: { collectedAt?: string | null; forcedMake?: string | null } = {}): ParsedPage {
  const state = extractState(html) as { listing?: { listing?: { ads?: unknown; totalElements?: unknown; totalPages?: unknown; pageNumber?: unknown } } } | null;
...
export function listingUrl({ make = null, region = null, page = 1, order = null }: { make?: string | null; region?: string | null; page?: number; order?: string | null } = {}) {
  let path = CAT_PATH;
  if (make) path += `${make}/`;
...
export function recordId(rec: OlxptRecord): string | null {
