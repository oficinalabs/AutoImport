// olxpt/schema.mjs — taxonomia de marcas + mapeamento de um anúncio SSR do olx.pt para o registo
// normalizado comum (+ extras que o OLX oferece).
//
// PORQUÊ: todos os coletores da AutoImport produzem o MESMO registo (ver lib/normalize.mjs), para
...
export const SLUG_TO_NAME = Object.fromEntries(MAKES.map((m) => [m.slug, m.name]));

// paramMap: array params[] → { key: {value, normalizedValue} } para acesso rápido.
function paramMap(params) {
  const m = {};
  for (const p of params || []) m[p.key] = p;
  return m;
}
const pv = (P, k) => cleanStr(P[k]?.value);           // valor legível
const pn = (P, k) => cleanStr(P[k]?.normalizedValue); // valor normalizado

// variant: título menos a marca (prefixo) e o modelo → sobra a versão/trim (ex. "1.2 Select").
function variantFromTitle(title, makeName, model) {
...
export function normalizeAd(a, { collectedAt = null, forcedMake = null } = {}) {
