// quoka/schema.ts — schema-alvo comum + mapeamento do card `article-item` (+ JSON-LD `Vehicle`)
// do quoka.de para o registo normalizado comum (ver lib/normalize.ts).
//
...
// Registo do quoka = os campos-base comuns + extras próprios do site (P2P).
export interface QuokaRecord extends CollectorRecord {
  source_site: string;
  id: string | null;
...
// Forma mínima do card `article-item` (produzido por parse.ts/parseCard) consumido aqui.
...
// Forma mínima do Vehicle do JSON-LD (Fonte 1, opcional). Payload externo dinâmico → só o que
// lemos, tudo opcional e `unknown` nas folhas, com narrowing no acesso.
interface LdVehicle {
  vehicleEngine?: { engineDisplacement?: { value?: unknown } };
  vehicleModelDate?: unknown;
  fuelType?: unknown;
  offers?: { price?: unknown };
  image?: { contentUrl?: unknown }[];
}
...
const SLUG_MAKE: Record<string, string> = {
...
export function makeFromSlug(slug: string | null): string | null {
  if (!slug) return null;
...
export function normalizeListing(card: RawCard, { ld = null, brandHint = null, collectedAt = null }: { ld?: LdVehicle | null; brandHint?: string | null; collectedAt?: string | null } = {}): QuokaRecord {
