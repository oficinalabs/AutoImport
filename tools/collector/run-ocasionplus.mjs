// run-ocasionplus.mjs — CLI da recolha batch do ocasionplus.com.
//
// Uso:
//   node run-ocasionplus.mjs --max-pages 3                # amostra (listagem geral)
//   node run-ocasionplus.mjs --brand audi --max-pages 2   # só uma marca (via path /coches-segunda-mano/audi)
...
import { crawl } from './ocasionplus/crawl.mjs';
...
async function main() {
  ...
  console.log(`=== ocasionplus.com${args.brand ? ` | marca ${args.brand}` : ''}`
  ...
  writeFileSync(join(outDir, 'ocasionplus-summary.json'), JSON.stringify(summary, null, 2));
  ...
  console.log(`top centros: ${top(stats.byCenter)}`);
  console.log(`\nNDJSON → ${ndjsonPath}`);
  console.log(`resumo → ${join(outDir, 'ocasionplus-summary.json')}`);
}
