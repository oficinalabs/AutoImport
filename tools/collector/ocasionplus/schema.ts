// ocasionplus/schema.ts — schema-alvo comum + mapeamento do JSON-LD do ocasionplus.com.
//
...
// Registo do ocasionplus = os campos-base comuns + extras próprios do site.
export interface OcasionplusRecord extends CollectorRecord {
  source_site: string;
  id: string | null;
  slug: string | null;
  center: string | null;
  ...
  price_finance: number | null;
  monthly: number | null;
  condition: string | null;
}

// Extras do card HTML (juntos ao JSON-LD pelo token do slug).
...
// Forma mínima de um `Vehicle` do JSON-LD que consumimos. Payload dinâmico → unknown + narrowing.
...
function powerFrom(...strs: unknown[]): number | null {
  for (const s of strs) {
...
export function normalizeVehicle(v: RawVehicle, { extras = {}, collectedAt = null }: { extras?: CardExtras; collectedAt?: string | null } = {}): OcasionplusRecord {
  const offer = v.offers || {};
  const make = cleanStr((v.brand as { name?: unknown } | null)?.name || v.brand);
  ...
  const slug = url ? new URL(String(url)).pathname.replace('/coches-segunda-mano/', '') : null;
...
export function tokenFromSlug(slug: unknown): string | null {
