// ocasionplus/parse.mjs — extração dos dados de uma página de listagem do ocasionplus.com.
//
...
import { normalizeVehicle, tokenFromSlug } from './schema.mjs';
  ...
  while ((m = re.exec(html)) !== null) {
    ...
    try {
      ...
      if (j && j['@type'] === 'ItemList' && Array.isArray(j.itemListElement)) {
        return j.itemListElement.filter((x) => x && x['@type'] === 'Vehicle');
      }
...
export function parseListingPage(html, { collectedAt = null } = {}) {
  ...
  const listings = vehicles.map((v) => {
    ...
    let token = null;
    try { token = url ? tokenFromSlug(new URL(url).pathname) : null; } catch { /* url inválido */ }
