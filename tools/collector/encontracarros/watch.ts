// encontracarros/watch.ts — recolha CONTÍNUA (polling) do encontracarros.pt. Deteta anúncios NOVOS e
// MUDANÇAS DE PREÇO, mantém uma "tabela" de estado (id→linha) e emite eventos para o sink (a DB fica
// isolada em lib/sink.ts).
//
...
import { PAGE_SIZE } from './crawl.ts';
...
type WatchRow = EncontracarrosRecord & { first_seen: string; last_seen: string };
...
export async function watch(config: WatchConfig) {
  ...
  const state = new Map<string, WatchRow>(Object.entries(persisted.rows || {}));
  let watermark: string | null = persisted.watermark || null;
