// caetano/http.mjs — cliente HTTP do coletor Caetano (rede de stands do Grupo Salvador Caetano /
...
export class HttpClient extends BaseClient {
  ...
  // ({ count, data:{ searchResult[] }, pagination }) ou null se esgotar as tentativas.
  async postSearch({ page = 1, numberElements = 250, sort = null, orderBy = null, body = {} } = {}) {
    ...
    for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
      ...
      try {
        ...
        if (!res.ok) { lastErr = `HTTP ${res.status}`; }
          ...
          lastErr = json?.message || 'resposta sem searchResult';
        }
