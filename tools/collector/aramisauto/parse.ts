// aramisauto/parse.ts — extração dos dados de uma página de listagem do aramisauto.com.
//
...
import { normalizeVehicle, type AramisautoRecord } from './schema.ts';

// Validador para o fetchText do lib: a página é útil só se trouxer o estado Nuxt.
export const temNuxt = (t: string) => t.includes('window.__NUXT__');

// Resultado do parse de uma página de listagem.
export interface ParsedPage {
  listings: AramisautoRecord[];
  total: number | null;
}

// Forma mínima da resposta de pesquisa do estado Nuxt (payload avaliado por vm → dinâmico).
interface SearchResponse { vehicles?: unknown; total?: unknown }
...
// Devolve o objeto de estado ou null se não existir/falhar. Payload dinâmico → `unknown`.
export function parseNuxt(html: string): unknown {
...
function searchResponse(nuxt: unknown): SearchResponse | null {
  return (nuxt as { data?: Array<{ displayedSearchVehicleResponse?: SearchResponse }> } | null)
    ?.data?.[0]?.displayedSearchVehicleResponse || null;
}
...
// proíbe `/*sort=*` / `/*orderBy`).
export function recordId(rec: AramisautoRecord): string | null {
