// aramisauto/watch.ts — recolha CONTÍNUA (polling) do aramisauto.com. Mesma lógica do
...
type WatchRow = AramisautoRecord & { first_seen: string; last_seen: string };
...
export async function watch(config: WatchConfig) {
  ...
  while (!stop) {
    ...
    let vistos = 0, novos = 0, alterados = 0, maxId: number | null = null;
