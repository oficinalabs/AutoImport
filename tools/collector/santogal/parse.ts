// santogal/parse.ts — extração dos dados de uma página de listagem do santogal.pt
...
import { normalizeListing, type SantogalRecord, type RawCard } from './schema.ts';
...
function infoByIcon(chunk: string, tipo: string): string | null {
...
function parsePushObject(chunk: string): Record<string, unknown> {
...
function parsePreco(chunk: string): { price: number | null; priceOld: number | null } {
...
function parseCard(chunk: string): RawCard {
...
export function parseListingPage(html: string, { collectedAt = null }: { collectedAt?: string | null } = {}): { listings: SantogalRecord[]; total: number | null } {
  const marca = /<div class="card_car\b/gi;
  const starts: number[] = [];
  let m;
  while ((m = marca.exec(html)) !== null) starts.push(m.index);
  const listings: SantogalRecord[] = [];
...
export function recordId(rec: SantogalRecord): string | null {
