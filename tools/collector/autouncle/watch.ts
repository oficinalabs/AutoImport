// autouncle/watch.ts — recolha CONTÍNUA (polling) do autouncle.pt. Mesma lógica do
...
type WatchRow = AutouncleRecord & { first_seen: string; last_seen: string };
...
export async function watch(config: WatchConfig) {
  ...
  while (!stop) {
    ...
    let vistos = 0, novos = 0, alterados = 0, minDias: number | null = null;
