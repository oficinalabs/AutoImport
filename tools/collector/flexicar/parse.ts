// flexicar/parse.ts — extração dos dados de uma página de listagem do flexicar.es.
//
...
import { normalizeVehicle, buildDealerMap, type FlexicarRecord } from './schema.ts';

// Resultado do parse de uma página de listagem.
export interface ParsedPage {
  listings: FlexicarRecord[];
  total: number | null;
}
...
export function parseListingPage(html: string, { collectedAt = null }: { collectedAt?: string | null } = {}): ParsedPage {
  const nd = extractNextData(html) as { props?: { pageProps?: { initialVehicles?: unknown; dealerships?: Array<Record<string, unknown>> } } } | null;
...
export function readTotal(html: string): number | null {
  const nd = extractNextData(html) as { props?: { pageProps?: { countVehicles?: number } } } | null;
...
export function extractBrandSlugs(html: string): string[] {
  const nd = extractNextData(html) as { props?: { pageProps?: { brands?: unknown } } } | null;
...
export function extractSitemapFacets(xml: string): string[] {
...
export function recordId(rec: FlexicarRecord): string | null {
