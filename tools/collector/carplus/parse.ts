// carplus/parse.ts — extração dos dados de uma página de listagem do carplus.pt (`/carros-usados/`).
//
...
import { normalizeVehicle, type CarplusRecord } from './schema.ts';
import { BASE } from './http.ts';

// Resultado do parse de uma página de listagem.
export interface ParsedPage {
  listings: CarplusRecord[];
  total: number | null;
}
...
function resolveNuxt(html: string): { arr: any[]; resolve: (i: unknown, depth?: number) => unknown } | null {
...
export function extractNuxtVehicles(html: string): Record<string, unknown>[] {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    const o = resolve(i) as Record<string, unknown> | null;
...
export function extractUrlByVin(html: string): Map<string, string> {
...
export function recordId(rec: CarplusRecord): string | null {
