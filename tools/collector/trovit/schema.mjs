// trovit/schema.mjs — schema-alvo comum + mapeamento do JSON-LD do coches.trovit.es.
//
...
function variantFrom(desc, cardTitle, name) {
...
export function normalizeCar(car, { extras = {}, collectedAt = null } = {}) {
  const desc = car.description || '';
  const offer = car.offers || {};
  const priceSpec = offer.priceSpecification || offer;
  ...
  return {
    make: cleanStr(car.brand?.name || car.brand),
