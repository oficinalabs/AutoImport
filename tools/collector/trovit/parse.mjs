// trovit/parse.mjs — extração dos dados de uma página de listagem do coches.trovit.es.
//
...
// anúncio está escondida atrás de um redirecionador (ver schema.mjs / http.mjs).
//
...
import { normalizeCar } from './schema.mjs';
...
export function extractCars(html) {
...
export function parseAgoMinutes(text) {
  if (!text) return null;
  ...
  if (/(\d+)\s*(?:mes|meses)/i.test(text)) { min += Number(RegExp.$1) * 30 * 24 * 60; achou = true; }
