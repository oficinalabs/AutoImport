// meinauto/watch.mjs — recolha CONTÍNUA (polling) do meinauto.de. Mesma lógica do autoboerse/
// watch.mjs: poll de X em X tempo, deteta NOVOS e MUDANÇAS DE PREÇO, mantém uma "tabela" de estado
// (id→linha) e emite eventos para o sink (DB isolada em lib/sink.mjs).
//
...
import { parseListingPage, temNuxtData, recordId } from './parse.mjs';
...
export async function watch(config) {
  ...
  const statePath = join(outDir, 'meinauto-state.json');
  const sink = new Sink(outDir, 'meinauto');
  ...
  console.log(`▶ watch meinauto.de | ${pages} pág recentes (sortBy=createdAt) | intervalo ${intervalMs / 1000}s`
