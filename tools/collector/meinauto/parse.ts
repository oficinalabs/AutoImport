// meinauto/parse.ts — extração dos dados de uma página de resultados do meinauto.de.
//
...
import { normalizeVehicle, type MeinautoRecord } from './schema.ts';

// Validador para o fetchText do lib: a página só é útil se trouxer o payload SSR do Nuxt 3.
export const temNuxtData = (t: string) => t.includes('__NUXT_DATA__');

// Resultado do parse de uma página de resultados.
export interface ParsedPage {
  listings: MeinautoRecord[];
  total: number | null;
  makes: string[];
}

// Forma mínima do contentor de resultados do estado Pinia (payload devalue → dinâmico).
interface ResultsMeta { totalResults?: unknown; counts?: { makes?: unknown } }
interface ResultsContainer { results?: unknown; meta?: ResultsMeta }
...
export function unflatten(arr: unknown): unknown {
  if (!Array.isArray(arr)) return null;
  const graph: any[] = arr;   // grafo devalue: caminhamos nós dinâmicos por índice
  const N = graph.length;
  const memo: unknown[] = new Array(N);
  const done: boolean[] = new Array(N).fill(false);

  function hydrate(i: unknown, stack: Set<number>): unknown {
    ...
    if (Array.isArray(v)) {
      if (v.length && typeof v[0] === 'string' && TAGS.has(v[0])) {
        ...
        if (tag === 'Set') { res = v.slice(1).map((x) => hydrate(x, stack)); }
          ...
          res = {};
          for (let k = 1; k + 1 < v.length; k += 2) res[hydrate(v[k], stack) as any] = hydrate(v[k + 1], stack);
...
function resultsContainer(root: unknown): ResultsContainer | null {
  return (root as { pinia?: { results?: ResultsContainer } } | null)?.pinia?.results || null;
}
...
export function parseListingPage(html: string, { collectedAt = null }: { collectedAt?: string | null } = {}): ParsedPage {
  ...
  const makesNode = cont.meta?.counts?.makes;
  const makes = makesNode && typeof makesNode === 'object' ? Object.keys(makesNode) : [];
...
export function recordId(rec: MeinautoRecord): string | null {
