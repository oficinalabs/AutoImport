// autosapo/crawl.mjs — recolha batch do auto.sapo.pt: paginação, dedupe, checkpoint, NDJSON.
...
function urlListagem(page, { slice = null, order = null } = {}) {
  const qs = [`p=${page}`];
...
export async function crawl(config) {
  ...
  for (let page = ckpt.donePages + 1; page <= CAP_PAGINAS; page++) {
    ...
    for (const r of listings) {
      ...
      seen.add(id);
      if (ckpt.detail) {                                             // enriquecimento opcional
