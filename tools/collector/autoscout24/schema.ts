// autoscout24/schema.ts — mapeia um objeto `listings[]` do __NEXT_DATA__ do AutoScout24
...
// Registo do AutoScout24 = os campos-base comuns + extras ricos próprios do site.
export interface Autoscout24Record extends CollectorRecord {
  source_site: string;
  id: string | null;
  city: string | null;
  street: string | null;
  ...
  super_deal: boolean | null;
  vat_label: boolean | null;
...
interface RawListing {
  ...
  wltpValues?: unknown[];
  images?: unknown;
  superDeal?: Record<string, unknown>;
...
export function normalizeListing(L: RawListing, { collectedAt = null }: { collectedAt?: string | null } = {}): Autoscout24Record {
  const v = L.vehicle || {};
  ...
  const isPrivate = /private/i.test(String(seller.type || ''));
  ...
  return {
    ...
    price_evaluation: Number.isInteger(price.priceEvaluation) ? Number(price.priceEvaluation) : null,
