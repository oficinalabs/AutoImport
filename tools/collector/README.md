...
## autouncle (vigésimo coletor — agregador MULTI-PAÍS)

Meta-motor/agregador dinamarquês **AutoUncle**, com avaliação de preço própria (**AutoScore** 1–5).
**14 domínios nacionais** (autouncle.pt/.de/.dk/.se/.it/.at/.es/.pl/.fi/.ro/.ch/.co.uk/.nl/.fr),
todos a MESMA SPA Next.js com o mesmo molde SSR — a tabela `MARKETS` em `autouncle/http.ts` captura
o que varia (domínio, locale, path do SRP, moeda, rótulo de país). Segue o **molde theparking**
(agregador). Investigação: [`../../research/autouncle-investigacao.md`](../../research/autouncle-investigacao.md).

- **Fonte = SSR do SRP do mercado (ex. `/pt/carros-usados`, `/da/brugte-biler`), dois blocos juntos
  pelo carId**: (1) **JSON-LD** (`@graph`→`ItemList.itemListElement[25].item` = `Product`+`Vehicle`
  rico + `numberOfItems` = total) para o catálogo; (2) **payload RSC** (`self.__next_f`, ~250 chunks)
  para o que falta a um agregador — a **fonte de origem** (`sourceName`), o **AutoScore** (`auRating`),
  a imagem real, a variante e os dias em stock. `source`=site/stand de origem;
  `source_site='autouncle.{tld}'`; `country`/`currency` do mercado.
- **Unidades por mercado**: `enginePower` vem em HP (PT) ou kW (DK…) como valor principal — o schema
...
agregado. Pronto exceto o upsert na DB ([`lib/sink.mjs`](lib/sink.mjs)). O `--full` fatia por marca;
as marcas densas (>2.500 por mercado) saturam o teto de paginação — corte fino por modelo é frágil
(slug do site ≠ config) e ficou por fazer. **Recência**: sem sort por data (robots) → proxy
`days_on_market`. Extras: `price_rating` (AutoScore), `estimated_price` (preço-justo AutoUncle),
`you_save`, `days_on_market`, `seller_type`, `source_slug`/`source_external_id`,
`power_hp`/`power_kw`, `co2`, `model_generation`.
...
## autoscout24.de — pan-europeu (vigésimo-quinto coletor — a alavanca nº1)

O **maior marketplace pan-europeu** de usados (~30M utilizadores/mês). Um único domínio cobre
**todos os países** via `cy=` multi-valor → **~2,15M viaturas** em `cy=D,A,B,E,F,I,L,NL`. Na **mesma
stack Scout24 do AutoTrader.nl** → segue o **molde `autotrader/`**. Investigação:
[`../../research/autoscout24-investigacao.md`](../../research/autoscout24-investigacao.md).

- **Fonte = `__NEXT_DATA__` (SSR)**: `props.pageProps.listings[]` + `numberOfResults`/`numberOfPages`
  + `taxonomy` (seed de ~290 marcas com IDs, sem hardcode). API interna (`/api/`, GraphQL) é
  robots-disallowed e desnecessária.
- **HTTP puro** (200, sem DataDome/Cloudflare-challenge) → **Scrapling desnecessário** (só
  contingência). `size=100` (5× o default). **Cap ~4.000 registos/query** (o `size` só troca
  throughput, não o teto).
- **`--full` = faceting adaptativo país×marca×faixa-preço**: para cada (país, marca) lê
  `numberOfResults`; ≤4.000 → pagina direto; >4.000 → sub-fatia por preço. Dedupe global por UUID,
  checkpoint/resume.
- **`--detail`** (1 req/anúncio, `/angebote/…`) enriquece fatias estreitas: equipamento (~55 itens),
  consumos, CO2, **todas as fotos**, carfax, **data de publicação**, contacto.
...
`--detail`, `--resume`, `--rate` (1500), `--out`; watch: `--country`, `--make`, `--pages` (1),
`--interval` (60), `--cycles`, `--online-since <1..14>`, `--rate`, `--out`. Saída em `out/`:
`autoscout24-*.ndjson` / `-summary.json` / `-checkpoint.json` (batch); `-state.json` /
`-events.ndjson` (watch). Pronto exceto o upsert na DB ([`lib/sink.mjs`](lib/sink.mjs)). Extras:
`price_evaluation` (avaliação de mercado do AS24, 1=muito bom … 5=alto — **ouro** p/ comparação),
`offer_type`, `seller_type`, `is_damaged`, `super_deal`, `model_id`, `image_count`.

> ✅ **Recência REAL** (ao contrário do AutoTrader.nl): o AS24 expõe `sort=age&desc=1` ("mais
> recentes primeiro") + filtro `onlineSince` (1–14 dias). O watch apanha inventário genuinamente
> novo, não um proxy.
>
> ⚠️ **Robots — transparência.** Ao contrário dos outros coletores (robots tolerantes), o AS24
> bloqueia bots-IA por nome (`Disallow: /` p/ ClaudeBot/GPTBot/CCBot) e proíbe `/lst?`+`/angebote/`
> no bloco `*`. Recolher com params livres (path `/lst/<marca>`) e UA de browser é uma **decisão
> explícita do utilizador**, documentada com honestidade. A guarda `robotsDisallow` mantém
> bloqueados **conta/sistema/API** (`/api/`, `/dealerarea/`, `/favorites`, `/ocs/api/`, …) — nunca
> lá tocamos.
...
autoscout24/              __NEXT_DATA__ SSR (pan-EU ~2,15M via cy; molde autotrader; +detail)
  http · parse · schema · crawl · detail · watch
...
run-autoscout24.mjs / watch-autoscout24.mjs    CLIs
```
