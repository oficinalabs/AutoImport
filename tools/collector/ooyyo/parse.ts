// ooyyo/parse.ts — extração dos dados de uma página de resultados (SRP) do Ooyyo (secção BE).
//
...
import { normalizeCard, type OoyyoCard, type OoyyoRecord } from './schema.ts';
import { BASE, BE_PARAMS } from './http.ts';

// Marca da qselements (top + restantes), para o plano de queries do --full.
export interface OoyyoMake { idMake: string; name: string; urlName: string; count: number }
// Resultado do parse da qselements.
export interface QsResult { seedUrl: string | null; total: number | null; makes: OoyyoMake[] }
// Formas mínimas (externas) do JSON da qselements — só o que lemos.
interface RawMake { idMake?: unknown; name: string; urlName: string; count?: number }
interface QsRaw { url?: string; count?: unknown; makes?: { top?: RawMake[]; black?: RawMake[] } }
// Resultado do parse de uma SRP.
export interface ParsedPage { listings: OoyyoRecord[]; nextUrl: string | null }
...
export function qselementsUrl(extra: Record<string, string> = {}): string {
...
export function parseQsElements(json: unknown): QsResult {
  let d: QsRaw;
  try { d = (typeof json === 'string' ? JSON.parse(json) : json) as QsRaw; } catch { return { seedUrl: null, total: null, makes: [] }; }
...
function sourceFromImage(imgUrl: string | null): { host: string | null; domain: string | null } {
...
function parseCard(chunk: string): OoyyoCard {
  const f = (re: RegExp, g = 1): string | null => { const m = re.exec(chunk); return m ? m[g].trim() : null; };
...
export function extractCards(html: string): OoyyoCard[] {
...
export function extractNextUrl(html: string): string | null {
...
export function recordId(rec: OoyyoRecord): string | null {
