// autopt/parse.ts — extração dos dados de uma página de listagem do auto.pt (`/carros-usados`).
//
...
import { normalizeListing, type Card, type VehicleLd, type AutoptRecord } from './schema.ts';
...
export function extractVehicleMap(html: string): Map<string, VehicleLd> {
  const map = new Map<string, VehicleLd>();
  let vehicles: any[] | null = null;   // array posicional (OfferCatalog)
  let items: any[] | null = null;      // array posicional (ItemList) com {position, url}
  ...
  while ((m = re.exec(html)) !== null) {
    ...
    if (j && j['@type'] === 'WebPage' && Array.isArray(j.mainEntity?.itemListElement)) {
      vehicles = j.mainEntity.itemListElement.map((o: any) => o.itemOffered || o);
    }
...
function parseCard(chunk: string, id: string): Card {
  ...
  const lis = [...ul.matchAll(/<li[^>]*>([\s\S]*?)<\/li>/g)].map((x) => textOf(x[1])).filter((x): x is string => Boolean(x));
  let fuel: string | null = null, year: number | null = null, km: number | null = null;
  for (const t of lis) {
...
export function parseListingPage(html: string, { collectedAt = null }: { collectedAt?: string | null } = {}): { listings: AutoptRecord[]; total: number | null } {
...
export function extractMakeSlugs(html: string): string[] {
...
export function recordId(rec: AutoptRecord): string | null {
