// autopt/schema.ts — schema-alvo comum + mapeamento do card `car_listing_entry` (+ Vehicle do
// JSON-LD) do auto.pt para o registo normalizado comum (ver lib/normalize.ts).
//
...
//       tem `url`/`id` → junta-se ao card por POSIÇÃO via o `ItemList` (ver parse.ts).
//
...
// Registo do auto.pt = os campos-base comuns + extras próprios do site.
export interface AutoptRecord extends CollectorRecord {
  source_site: string;
  id: string | null;
  owner_type: string;
  dealer: string | null;
...
// Card extraído do HTML (produzido em parse.parseCard).
...
// Vehicle do JSON-LD (payload externo → campos opcionais; brand/mileage/condition podem ser
// objeto OU escalar, tratados no runtime como no JS original).
export interface VehicleLd {
  ...
  fuelType?: unknown;
  offers?: { price?: unknown; priceSpecification?: { price?: unknown } };
  itemCondition?: { name?: unknown };
  image?: unknown;
}
...
function yearFromModelDate(d: unknown): number | null {
  const m = /^(\d{4})/.exec(String(d ?? ''));
  return m ? Number(m[1]) : null;
}

// Normaliza um card (Fonte 1) + o Vehicle do JSON-LD (Fonte 2, opcional) → registo comum + extras.
export function normalizeListing(card: Card, { veh = null, collectedAt = null }: { veh?: VehicleLd | null; collectedAt?: string | null } = {}): AutoptRecord {
