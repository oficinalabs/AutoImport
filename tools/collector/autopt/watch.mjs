// autopt/watch.mjs — recolha CONTÍNUA (polling) do auto.pt. Mesma lógica do autocasion/watch.mjs:
...
export async function watch(config) {
  ...
  const statePath = join(outDir, 'autopt-state.json');
  const sink = new Sink(outDir, 'autopt');
  ...
  console.log(`▶ watch auto.pt | ${pages} pág (ordem default) | intervalo ${intervalMs / 1000}s`
