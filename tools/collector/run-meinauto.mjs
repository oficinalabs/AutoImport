// run-meinauto.mjs — CLI da recolha batch do meinauto.de (Gebrauchtwagen / usados).
//
// Uso:
//   node run-meinauto.mjs --max-pages 3                 # amostra (listagem geral de usados)
//   node run-meinauto.mjs --brand Audi --max-pages 2    # só uma marca (via makes=Audi)
...
import { HttpClient } from './meinauto/http.mjs';
import { crawl } from './meinauto/crawl.mjs';
...
async function main() {
  ...
  console.log(`=== meinauto.de (Gebrauchtwagen)${args.brand ? ` | marca ${args.brand}` : ''}`
  ...
  writeFileSync(join(outDir, 'meinauto-summary.json'), JSON.stringify(summary, null, 2));
  ...
  console.log(`\nNDJSON → ${ndjsonPath}`);
  console.log(`resumo → ${join(outDir, 'meinauto-summary.json')}`);
}
