// autoline/schema.ts — schema-alvo comum + mapeamento do autoline.pt (marketplace Via Mobilis).
//
...
//
// FONTE (ver parse.ts): CARD sempre presente (make/ano/km/combustível/Euro/local/vendedor/imagem/
...
// Registo do autoline = os campos-base comuns + extras próprios do site.
export interface AutolineRecord extends CollectorRecord {
  source_site: string;
  id: string | null;
  dealer: string | null;
  is_auction: boolean;
...
// Card da listagem (fonte primária, ver parse.ts). Só os campos que lemos.
...
// Forma mínima do `Product` do JSON-LD (enriquecimento). Payload externo → campos opcionais.
interface Offer {
  price?: unknown;
...
interface AdditionalProperty { name?: unknown; value?: unknown }
...
const PAIS_PT_EN: Record<string, string> = {
...
export function ccToCountry(cc: string | null) {
...
function localDoCard(locText: string | null, countryCode: string | null) {
...
export function createdAtFromId(id: unknown) {
  const d = String(id ?? '');
...
const propOf = (p: Product | null, nome: string): string | null => {
...
function precoDoCard(txt: unknown) {
  const s = String(txt ?? '');
...
export function normalizeCard(card: Card, p: Product | null = null, { collectedAt = null, countryCode = null }: { collectedAt?: string | null; countryCode?: string | null } = {}): AutolineRecord {
  const offer: Offer = (p && p.offers) || {};
  ...
  const id = card.id || (/--(\d+)(?:$|[?#])/.exec(String(card.detail_url || p?.url || '')) || [])[1] || null;
