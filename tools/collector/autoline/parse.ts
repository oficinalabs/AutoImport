// autoline/parse.ts — extração dos dados de uma página de listagem do autoline.pt.
//
...
import { normalizeCard, type Card, type Product, type AutolineRecord } from './schema.ts';
...
export function extractProducts(html: string) {
...
export function extractCards(html: string) {
  const cards: Card[] = [];
  ...
  for (let i = 0; i < inicios.length; i++) {
    ...
    const grab = (re: RegExp) => { const x = re.exec(flat); return x ? x[1] : null; };
...
export function parseListingPage(html: string, { collectedAt = null, countryCode = null }: { collectedAt?: string | null; countryCode?: string | null } = {}) {
...
export function readTotal(html: string) {
...
export function extractCountryFacets(html: string) {
  const out = new Map<string, { cc: string; slug: string }>();
...
export function recordId(rec: AutolineRecord) {
