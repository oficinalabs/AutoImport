// run-encontracarros.mjs — CLI da recolha batch do encontracarros.pt (agregador PT; via sitemap +
// páginas de detalhe SSR).
//
// Uso:
//   node run-encontracarros.mjs --max-pages 3                  # amostra: ~90 anúncios mais recentes
//   node run-encontracarros.mjs --brand bmw --max-pages 2      # só uma marca (filtro do slug)
//   node run-encontracarros.mjs --district porto --max-pages 2 # só um distrito (filtro do slug)
//   node run-encontracarros.mjs --since 2026-07-12 --max-pages 5   # só lastmod recente
//   node run-encontracarros.mjs --full                         # cobertura máxima (~50k, longo)
...
import { HttpClient } from './encontracarros/http.mjs';
import { crawl } from './encontracarros/crawl.mjs';
