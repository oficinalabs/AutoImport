...
page.on('request', (req) => {
  ...
  let post = null; try { post = req.postData()?.slice(0, 500); } catch {}
...
try { await page.evaluate(() => window.ctrl && window.ctrl.set_pageReload(2)); } catch (e) { log('pageReload err: ' + e.message); }
await sleep(4000);
...
try {
  ...
  await page.evaluate(() => window.ctrl && window.ctrl.set_criteria && window.ctrl.set_criteria('id_marque', 74, 'BMW'));
} catch (e) { log('filter err: ' + e.message); }
await sleep(4000);
...
} catch (e) { log('detail nav err: ' + e.message); }
await sleep(3500);
